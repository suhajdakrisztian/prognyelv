...
template <class TYPE, class PREDICATE,
          class MSET_COMP_FST = std::less<TYPE>,
          class MSET_COMP_SND = std::less<TYPE>
         >
...
class multisets_predicate_view
{
  ...
  public:
                             ...
                             std::multiset<TYPE, MSET_COMP_SND>& second_input_mset, PREDICATE p) : _predicate(p)
    {
      ...
      std::copy_if(first_input_mset.begin(), 
      first_input_mset.end(), 
                   std::inserter(_valid_elements, _valid_elements.begin()), 
                   _predicate );
      ...
      std::copy(second_input_mset.begin(), 
                second_input_mset.end(),
                std::inserter(_original_values_second_mset, 
                              _original_values_second_mset.begin()) );

      std::copy_if(second_input_mset.begin(),
                   second_input_mset.end(),
                   std::inserter(_valid_elements,
                                 _valid_elements.begin()),
                   _predicate );
      ...
      std::copy(_valid_elements.begin(), 
                _valid_elements.end(),
                std::inserter(first_input_mset, 
                              first_input_mset.begin()));

      std::copy(_invalid_elements.begin(), 
                _invalid_elements.end(),
                std::inserter(second_input_mset, 
                              second_input_mset.begin()));
    }
    ...
    int count(const TYPE& element_to_count) const
    {
